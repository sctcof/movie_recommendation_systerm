...
def get_movielens_data(data_path, load_cache=False, seq_max_len=50):
    ...
    if load_cache:  #if you have run this script before and saved the preprocessed data
        ...
        # print(df_train[df_train['user_id']==5306].head())
        ##列表匹配模型的处理方式
        # x_train = gen_model_input(df_train, user_profile, user_col, item_profile, item_col
        #                           , seq_max_len=seq_max_len, padding='post', truncating='post')
        # y_train = np.array([0] * df_train.shape[0])  # label=0 means the first pred value is positiva sample
        # x_test = gen_model_input(df_test, user_profile, user_col, item_profile, item_col
        #                          , seq_max_len=seq_max_len, padding='post', truncating='post')
        # mode!=0 则下面打开
        # y_train = np.array([0] * df_train.shape[0])  #label=0 means the first pred value is positiva sample
        x_train = gen_model_input(df_train, user_profile, user_col, item_profile, item_col
    ...
    # user_cols = ['user_id', 'gender', 'age', 'occupation', 'zip_code'] #原来的
    ...
    return user_features, history_features, item_features, x_train, y_train, all_item, test_user
...
def main(dataset_path, model_name, epoch, learning_rate, batch_size, weight_decay, device, save_dir, seed, seq_max_len=50):
    ...
    user_features, history_features, item_features, x_train, y_train, all_item, test_user \
        = get_movielens_data(dataset_path, seq_max_len=seq_max_len)
    dg = MatchDataGenerator(x=x_train, y=y_train)
    model = MIND(user_features, history_features, item_features,None,
                 max_length=seq_max_len, temperature=0.02,task_type='CTR')
    # print(model)
    ...
    trainer.fit(train_dl,model_name='model_5.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epoch', type=int, default=5)  #10
