...
from model.basic.layers import MLP, EmbeddingLayer, MultiInterestSA, CapsuleNetwork
...
class MIND(torch.nn.Module):
    ...
    def __init__(self, user_features, history_features, item_features, neg_item_feature, max_length
                 , temperature=1.0, interest_num=4,task_type=None):
        super().__init__()
        ...
        self.mode = None
        self.ctr_pred_linear = torch.nn.Linear(16,1)
    ...
    def forward(self, x):
        ...
        if self.task_type == 'CTR':
            y = self.ctr_pred_linear(y)
            y = self.sigmoid(y).view(-1)
        return y
    ...
    def user_tower(self, x):
        ...
        # print("history_features:", self.history_features)
        ...
        # print("multi_interest_emb:", multi_interest_emb.shape)
        #
        # print("input_user1:", input_user.shape)

        input_user = torch.cat([input_user,multi_interest_emb],dim=-1)

        # print("input_user2:", input_user.shape)
    ...
    def item_tower(self, x):
        if self.mode == "user":
            return None
        pos_embedding = self.embedding(x, self.item_features, squeeze_dim=False)  #[batch_size, 2, embed_dim]
        pos_embedding = F.normalize(pos_embedding, p=2, dim=-1)  # L2 normalize
        pos_embedding = pos_embedding.transpose(-1,-2) #[batch_size, embed_dim,2]
        ...
        if self.neg_item_feature is None:
            return pos_embedding  # [batch_size, 1, embed_dim]
        else:
