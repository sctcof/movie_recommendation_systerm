...
from model.basic.callback import EarlyStopper
from model.basic.loss_func import BPRLoss
...
class MatchTrainer(object):
    ...
    def fit(self, train_dataloader, val_dataloader=None,model_name="model.pth"):
        ...
        torch.save(self.model.state_dict(), os.path.join(self.model_path,
                                                            model_name))  #save best auc model
    ...
    def inference_embedding(self, model, mode, data_loader, model_path):
        ...
        model.mode = mode
        model.load_state_dict(torch.load(os.path.join(model_path, "model_5.pth")))
